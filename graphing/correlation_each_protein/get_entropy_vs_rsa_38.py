...
def plt_plot_color1(x, y, ax, xlabel, ylabel):
	...
	ax.set_yticks([0, 5.0, 10.0, 15.0, 20.0])
...
def plt_plot_color3(x, y, ax, xlabel, ylabel):
	x = np.array(x, dtype='|S32')
	y = np.array(y, dtype='|S32')
	x = x.astype(np.float)
	y = y.astype(np.float)

	d3, = ax.plot(x, y, 'o', color="magenta", markersize=1, markeredgecolor="magenta")
	ax.spines['top'].set_visible(False)
	ax.spines['right'].set_visible(False)
	ax.spines['bottom'].set_linewidth(0.5)
	ax.spines['left'].set_linewidth(0.5)
...
for protein in PDBS:
	...
	p1 = plt.subplot(10,12,(1+(count-1)*3))
	p2 = plt.subplot(10,12,(2+(count-1)*3))
	p3 = plt.subplot(10,12,(3+(count-1)*3))

	subplots_adjust(left=0.08, bottom=0.1, right=0.96, top=0.92, wspace=0.2,hspace=0.7)
	
	d1 = plt_plot_color1(designed_RSA_array_rosetta, designed_entropy_array_rosetta, p1, "", "")
	#p1.text(0, 3, "FB", fontweight = 'bold', ha = 'center', va = 'center', fontsize = 18)
	p1.set_title(pdb_id + '_' + chain_id, fontsize=6, fontweight = 'bold')

	d2 = plt_plot_color2(designed_RSA_array_evolved, designed_entropy_array_evolved, p2, "", "")
	#p2.text(0, 3, "ES", fontweight = 'bold', ha = 'center', va = 'center', fontsize = 18)
	
	d3 = plt_plot_color3(natural_RSA_array, natural_entropy_array, p3, "", "")
	...
	if count == 35 or count == 36 or count == 37 or count ==38:
		p1.set_xlabel("RSA",fontsize=6, fontweight = 'bold')
		p1.set_xticklabels(["0", "", "0.5", "", "1"], fontsize=5)
		p2.set_xlabel("RSA",fontsize=6, fontweight = 'bold')
		p2.set_xticklabels(["0", "", "0.5", "", "1"], fontsize=5)
		p3.set_xlabel("RSA",fontsize=6, fontweight = 'bold')
		p3.set_xticklabels(["0", "", "0.5", "", "1"], fontsize=5)
