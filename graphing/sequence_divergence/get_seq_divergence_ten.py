...
#PDBS = ["1ci0A"]
PDBS = ["1b4tA", "1ci0A", "1efvB", "1g58B", "1gv3A", "1hujA", "1hurA", "1ibsA", "1jlwA", "1ky2A"]
FASTA_PATH = "/Users/qian/Desktop/evol_sim_vs_rosetta/"
NATURAL_FILE_PATH = "sequences/aligned_sequences/"
NATURAL_FILE_POSFIX = "Aligned_Sequences.fasta"
ROSETTA_FILE_PATH = "sequences/designed_sequences_fasta/"
ROSETTA_FILE_POSFIX = "designed_seqs.fasta"
EVOLVED_FILE_PATH = "sequences/designed_sequences_fasta/"
EVOLVED_FILE_POSFIX = "evolved_seqs.fasta"
EVOLVED_FILE_PATH_2 = "sequences_ten/designed_sequences_fasta/"
...
for protein in PDBS:
	...
	evolved2_file = FASTA_PATH + EVOLVED_FILE_PATH_2 + pdb_id + "_" + chain_id + "_" + EVOLVED_FILE_POSFIX
	...
	evolved2 = cal_divergence(evolved2_file, natural_file, False)
	
	rosetta_array.append(mean(rosetta))
	evolved_array.append(mean(evolved))
	natural_array.append(mean(natural))
	evolved2_array.append(mean(evolved2))
	
all_data = []
all_data.append(rosetta_array)
all_data.append(evolved2_array)
all_data.append(evolved_array)
all_data.append(natural_array)


#open file name
divergence_filename = "mean_seq_divergence_ten.csv"
divergence_file = open(divergence_filename, "w")
divergence_file.write("PDB\tchain\trosetta\tevolved_from_designed\tevolved\tnatural\n")
divergence_length = len(pdb_names)
length_counter = 0
while(length_counter<divergence_length):
    rosetta = rosetta_array[length_counter]
    evolved2 = evolved2_array[length_counter]
    evolved = evolved_array[length_counter]
    natural = natural_array[length_counter]
    divergence_filestring = pdb_names[length_counter] +"\t" + chain_names[length_counter] + "\t" + str(rosetta) + "\t" + str(evolved2) + "\t" + str(evolved) + "\t" + str(natural) + "\n"
...
plt.ylim(0, 1)
plt.xticks([1, 2, 3, 4], ["designed", "evolved \n from design", "evolved", "natural"])
gca().set_yticklabels(['{:.0f}%'.format(x*100) for x in gca().get_yticks()]) 

save_fig_title = "mean_divergence_ten" + ".eps"
