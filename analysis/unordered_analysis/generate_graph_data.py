...
for path, names, filename in os.walk('.',False): #Searchs for all the *.dat files for the #natural protein 
    for file in filename:
        if(re.search(searchStr, file)!=None):
            ...
            iCN13 = cn13_data[1]
            iCN13_array = analysis_functions.make_array(iCN13)
            mean_iCN13_values.append(mean(iCN13_array))
    
            wcn_data = analysis_functions.get_wcn_values(pdb_id, chain_id)
            iWCN = wcn_data[1]
            iWCN_array = analysis_functions.make_array(iWCN)
            ...
            #Calculates the iCN13 & entropy correlation
            [natural_icn_entropy_corr,p_value] = spearmanr(iCN13_array, natural_entropy_array) 
            natural_icn_entropy_corr = float(natural_icn_entropy_corr)
            natural_cor_entropy_icn_values.append(natural_icn_entropy_corr)
        

            [designed_icn_entropy_corr_evolved,p_value] = spearmanr(iCN13_array, designed_entropy_array_evolved) 
            designed_icn_entropy_corr_evolved = float(designed_icn_entropy_corr_evolved)
            designed_cor_entropy_icn_values_evolved.append(designed_icn_entropy_corr_evolved)

            [designed_icn_entropy_corr_rosetta,p_value] = spearmanr(iCN13_array, designed_entropy_array_rosetta) 
            designed_icn_entropy_corr_rosetta = float(designed_icn_entropy_corr_rosetta)
            designed_cor_entropy_icn_values_rosetta.append(designed_icn_entropy_corr_rosetta)
            ...
            natural_file.write("iCN13\t" + analysis_functions.dump_csv_line3(iCN13_array))
...
natural_mean_file.write("PDB\tchain\tmean_RSA\tmean_entropy\tcor_entropy_RSA\tsplit_mean_KL\tcor_entropy_icn\tcor_entropy_iwcn\n")
...
while(length_counter<natural_name_length):
    ...
    natural_filestring = pdb_names[length_counter] +"\t" + chain_names[length_counter] + "\t" + str(natural_mean_RSA) + "\t" + str(natural_mean_entropy) + "\t" + str(natural_cor_entropy_RSA) + "\t" + str(natural_mean_split_KL) + "\t" + str(natural_cor_entropy_icn) + "\t" + str(natural_cor_entropy_iwcn) + "\n"
...
designed_mean_file.write("PDB\tchain\tmean_RSA\tmean_entropy\tcor_entropy_RSA\tmean_KL\tcor_entropy_icn\tcor_entropy_iwcn\n")
...
while(length_counter<designed_name_length):
    ...
    designed_cor_entropy_icn = designed_cor_entropy_icn_values_rosetta[length_counter]
    designed_cor_entropy_iwcn = designed_cor_entropy_iwcn_values_rosetta[length_counter]
    designed_filestring = pdb_names[length_counter] + "\t" + chain_names[length_counter] + "\t" + str(designed_mean_RSA) + "\t" + str(designed_mean_entropy) + "\t" + str(designed_cor_entropy_RSA) + "\t" + str(designed_mean_KL) + "\t" + str(designed_cor_entropy_icn) + "\t" + str(designed_cor_entropy_iwcn) + "\n"
...
while(length_counter<designed_name_length):
    ...
    designed_cor_entropy_icn = designed_cor_entropy_icn_values_evolved[length_counter]
